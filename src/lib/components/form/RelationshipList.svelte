	...
	<div class="flex flex-col gap-1.5">
		...
		<div class="flex flex-col gap-1.5">
			{#each filteredRelationships as rel (rel.id)}
				...
				<div
					class="min-w-0 overflow-hidden rounded-lg border transition-all {isSelected ? 'border-[var(--ui-text-secondary)] bg-[var(--ui-bg-tertiary)]' : 'border-[var(--ui-border-light)] bg-[var(--ui-bg-secondary)] hover:border-[var(--ui-border)]'}"
				>
					<!-- Header row -->
					<div class="flex items-center justify-between px-3 py-2">
						<button
							class="min-w-0 flex-1 text-left text-sm"
							onclick={() => diagram.selectRelationship(rel.id)}
						>
						...
						<button
							...
							onclick={() => diagram.removeRelationship(rel.id)}
					...
					{#if isSelected}
						<div class="flex flex-col gap-2.5 border-t border-[var(--ui-border-light)] px-3 py-2.5 animate-slide-up">
							<!-- Name -->
